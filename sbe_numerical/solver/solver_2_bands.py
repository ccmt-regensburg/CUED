...
from observables_2_bands import current_in_path
from dipole_numerical import diagonalize, dipole_elements

def sbe_solver_2_bands(params):
    # RETRIEVE PARAMETERS
    ###########################################################################
    # Flag evaluation
    ...
    fnumba = make_fnumba(E_dir, gamma1, gamma2, electric_field,
    ...
    # Set after first run
    emission_exact_path = np.zeros([params.Nt, 2], dtype=np.float64)
    ...
    for Nk2_idx, path in enumerate(paths):
        ...
        solver.set_initial_value(y0, t0)\
            .set_f_params(path, paths, Nk2_idx, dk, ecv_in_path, dipole_in_path, y0)
...
def make_fnumba(E_dir, gamma1, gamma2, electric_field, gauge,
                do_semicl):
    if gauge == 'length':
    ...
    def fnumba(t, y, path, paths, path_idx, dk, ecv_in_path, dipole_in_path, y0):
        # x != y(t+dt)
        ...
        for k in range(Nk_path):
            ...
            wr_d_diag = (dipole_in_path[k, 0, 0] - dipole_in_path[k, 1, 1])*electric_f
