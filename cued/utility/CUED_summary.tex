...
\caption{Band structure~$\epsilon_n(\bk)$ as computed from the Hamiltonian~$h(\bk)$, see Eq.~\eqref{e3}. }
...
\begin{figure}
\centering
\setlength\figureheight{10.5cm} 
\setlength\figurewidth{0.95\textwidth}
\input{Emission_total.tikz}
\\[1.5em]
\input{Emission_total_hann_parzen.tikz}
\caption{Total emission intensity from the irradiated material computed from Eq.~\eqref{emission}. 
%
In the lower graph, the emission computed from $\bj_w(\omega)$ is sketched where the Hann and the Parzen window have been used as window $w(t)$. 
%
This is a check: If both Fourier transforms coincide, the Fourier transform is converged with respect to the time window $[-t_0,t_0]$.
%
If the Fourier transforms with Hann and Parzen window differ, increase~$t_0$.
%
The frequency is given by $f\eqt\omega/(2\pi)$.
}
    \label{fig:emissiontotal}
\end{figure}
\begin{figure}
\centering
\setlength\figureheight{10.5cm} 
...
\caption{Emission intensity of the polarization parallel and orthogonal to the incoming electric field from the irradiated material computed from Eq.~\eqref{emissiondecomp2}.
%
...
%
The emission is sketched in Fig.~\ref{fig:emissiontotal} including a test of the window function.
%
%
