...
def label_inner(ax, idx=None, color='black'):
    ...
    '''
    if idx is None:
        ax.text(0.02, 0.88, r'{}'.format(labels[plt_fig_idx_inner.counter]), transform=ax.transAxes, color=color)
        label_inner.counter += 1
    else:
        ax.text(0.02, 0.88, r'{}'.format(labels[idx]), transform=ax.transAxes, color=color)
