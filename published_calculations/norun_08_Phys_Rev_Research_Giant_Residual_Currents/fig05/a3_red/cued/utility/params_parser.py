...
class ParamsParser():
    ...
    def __field(self, UP):
        ...
        if 'electric_field_function' in UP:
            ...
            if True:
                nt = int(-2*self.t0/self.dt)
                t_list = np.linspace(self.t0, -self.t0, num=nt)
                Et = self.E0*np.exp(-t_list**2/self.sigma**2) \
                        * np.sin(2.0*np.pi*self.f*t_list*(1 + self.chirp*t_list) + self.phase)
                Emax = np.amax(Et)
                Emin = - np.amin(Et)
                if Emax > Emin:
                    self.E0 *= UP['E0']/Emax * CoFa.MVpcm_to_au
                else:
                    self.E0 *= UP['E0']/Emin * CoFa.MVpcm_to_au
